...
namespace AerSpeech
{
    ...
    public class EliteListing
    {
        ...
        public EliteStation Station; //was commented out?  by me?
    ...
    public class NMDB
    {
        ...
        public NMDB(string pathToJsonFiles = @"json\")
        {
            ...
            if (File.Exists(systemsJson) && File.Exists(commoditiesJson) && File.Exists(stationsJson))
            {
                ...
                {
                    NMDebug.LogError("Encountered a problem parsing EDDB json files");
                    NMDebug.LogException(e);
                }
            }
            else
            {
                NMDebug.LogError("Could not find JSON files!");
            }
            Loaded = true;
        }
...
#region JSON Parsing
        ...
        private void _ParseSystems(string json)
        {
            ...
            foreach (JObject jo in ja)
            {
                ...
                {
                    NMDebug.LogError("Malformed/Unexpected System JSON data, " + e.Message);
                }
        ...
        {
            NMDebug.Log("Loading Commodities...");
            Stopwatch timer = new Stopwatch();
            timer.Start();
            JArray ja = JArray.Parse(json);
            ...
            foreach (JObject jo in ja)
            {
                ...
                {
                    NMDebug.LogError(@"Error formatting Average Price for "+ ec.Name +", " + e.Message);
                    ec.AveragePrice = -1;
                }
        ...
        private void _ParseStations(string json)
        {
            ...
            while(jsonReader.Read())
            {
                switch(jsonReader.TokenType)
                {
                    ...
                    case JsonToken.StartObject:
                        ...
                        {
                            NMDebug.LogError("Encountered a problem parsing stations.");
                            NMDebug.LogException(e);
                        }
                        break;
                    case JsonToken.EndArray:
                        ...
                        break;
                    default:
                        NMDebug.LogError("Unknown JSON TokenType: " + jsonReader.TokenType);
                        break;
                }   
            }
            ...
            if(arrayDepth != 0)
            {
                NMDebug.LogError("Malformed JSON parsing - arrayDepth == " + arrayDepth + " at end of parse");
            }
        }
        ...
        private EliteStation _ParseJsonStation(JsonTextReader jsonReader)
        {
            ...
            if (jsonReader.TokenType != JsonToken.StartObject)
                NMDebug.LogError("Malformed JSON parsing - _ParseJsonStation must be called on a StartObject token");
        ...
        private List<EliteListing> _ParseJsonListing(JsonTextReader jsonReader)
        {
            ...
            {
                NMDebug.LogError("_ParseJsonListing must be called at the start of an Array");
                return null;
            }
                    ...
                    case JsonToken.PropertyName:
                        switch (jsonReader.Value.ToString())
                        {
                            ...
                            case "update_count":
                                ...
                                break;
                            default:
                                NMDebug.LogError("Unknown JSON property name: " + jsonReader.Value.ToString());
                                break;
                        }
                        break;
                    case JsonToken.EndArray:
                        break;
                    default:
                        NMDebug.LogError("Unknown token type in listing list, " + jsonReader.TokenType);
                        break;
                }
            }
            return listings;
        ...
        private List<EliteCommodity> _ParseJsonCommodities(JsonTextReader jsonReader)
        {
            ...
            if(jsonReader.TokenType != JsonToken.StartArray)
            {
                NMDebug.LogError("_ParseJsonCommodities must be called at the start of an Array");
                return null;
            }
            ...
            while(jsonReader.TokenType != JsonToken.EndArray)
            {
                ...
                switch(jsonReader.TokenType)
                {
                    ...
                    case JsonToken.EndArray:
                        break;
                    default:
                        NMDebug.LogError("Unknown token type in commodities list, " + jsonReader.TokenType);
                        break;
                }
            }
        ...
        private List<string> _ParseJsonEconomies(JsonTextReader jsonReader)
        {
            ...
            if (jsonReader.TokenType != JsonToken.StartArray)
            {
                NMDebug.LogError("_ParseJsonEconomies must be called at the start of an Array");
                return null;
            }
            ...
            while (jsonReader.TokenType != JsonToken.EndArray)
            {
                ...
                switch (jsonReader.TokenType)
                {
                    ...
                    case JsonToken.EndArray:
                        break;
                    default:
                        NMDebug.LogError("Unknown token type in economies list, " + jsonReader.TokenType);
                        break;
                }
            }
            return econs;
        ...
        public void DBG_CompileGrammars()
        {
            ...
            NMDebug.Log("Compiling grammars...");
            NMDebug.Log("Compiling grammars for systems...");
            ...
            NMDebug.Log("Compiling grammars for commodities...");
            ...
            NMDebug.Log("Compiling grammars for stations...");
...
#region Utility Methods
        ...
        public EliteStation GetStation(EliteSystem es, string stationName)
        {
            ...
            {
                NMDebug.LogError(@"Found " + matchingStations.Count +" stations with the name '" + stationName + "' in system '" + es.Name + "'");
            }
